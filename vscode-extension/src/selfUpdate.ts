...
export class SelfUpdate {
    ...
    async checkForUpdates(): Promise<UpdateInfo> {
        try {
            ...
            return {
                ...
                latestVersion,
                updateAvailable,
                downloadUrl: releaseInfo.assets?.find((a: any) => a.name.endsWith('.vsix'))?.browser_download_url,
     ...
     */
    async update(progressCallback?: (msg: string) => void): Promise<boolean> {
        try {
            ...
            if (updateInfo.downloadUrl) {
                ...
                if (vsixPath) {
                    ...
                    const reload = await vscode.window.showInformationMessage(
                        `Mendix Widget Agent updated to ${updateInfo.latestVersion}. Reload to apply?`,
                        'Reload Now',
                        'Later'
                    );
     ...
     */
    getLocalVsixPath(): string | undefined {
     ...
     */
    getManualUpdateInstructions(): string {
        return `
...
`;
    ...
    private async downloadVsix(url: string, progressCallback?: (msg: string) => void): Promise<string | null> {
    ...
    private async buildFromSource(progressCallback?: (msg: string) => void): Promise<boolean> {
